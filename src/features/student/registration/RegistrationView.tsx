  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        {/* Search bar moved to filter section */}
      </div>
      ...
      {/* 3. DANH SÁCH MÔN HỌC THUỘC KHUNG CHƯƠNG TRÌNH */}
      <Card className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        
        <div className="p-4 bg-slate-50/50 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2">
            <h3 className="text-sm font-bold text-slate-800">Khung chương trình & Đăng ký</h3>
            <p className="text-xs italic text-slate-500">(Ghi chú: Nhấn vào Tên học phần màu xanh để xem danh sách lớp mở)</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-grow max-w-md">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-lg border border-slate-200 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 bg-white text-slate-800"
              />
            </div>
            <select 
              value={semesterFilter}
              onChange={(e) => setSemesterFilter(e.target.value === 'ALL' ? 'ALL' : Number(e.target.value))}
              className="text-sm border border-slate-200 rounded-lg px-3 py-2 bg-white text-slate-700 outline-none focus:ring-1 focus:ring-blue-500 min-w-[120px]"
            >
              <option value="ALL">Tất cả các kỳ</option>
              {Array.from({ length: 8 }).map((_, i) => (
                <option key={i} value={i + 1}>Kỳ {i + 1}</option>
              ))}
            </select>
            <select 
              value={subjectTypeFilter}
              onChange={(e) => setSubjectTypeFilter(e.target.value)}
              className="text-sm border border-slate-200 rounded-lg px-3 py-2 bg-white text-slate-700 outline-none focus:ring-1 focus:ring-blue-500 min-w-[150px]"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-3 font-bold text-slate-600 w-12 text-center">STT</th>
                  <th className="px-4 py-3 font-bold text-slate-600 w-32">Mã học phần</th>
                  <th className="px-4 py-3 font-bold text-slate-600">Tên học phần</th>
                  <th className="px-4 py-3 font-bold text-slate-600 w-24 text-center">Kỳ</th>
                  <th className="px-4 py-3 font-bold text-slate-600 w-32 text-center">Loại môn</th>
                  <th className="px-4 py-3 font-bold text-slate-600 w-20 text-center">Tín chỉ</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {filteredCurriculum.length === 0 ? (
                  ...
                  filteredCurriculum.map((subjectGroup: any, idx: number) => {
                    ...
                    const typeColor = 
                      subjectGroup.type === 'COMPULSORY' ? 'text-rose-600 bg-rose-50' :
                      subjectGroup.type === 'ELECTIVE' ? 'text-indigo-600 bg-indigo-50' :
                    ...
                    return (
                      <React.Fragment key={subjectGroup.subjectId}>
                        <tr className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3 font-mono text-xs">{subjectGroup.subjectCode || subjectGroup.subjectId}</td>
                          <td className="px-4 py-3">
                            {subjectGroup.classes.length > 0 ? (
                              <button
                                onClick={() => toggleSubject(subjectGroup.subjectId)}
                                className="text-blue-600 font-semibold hover:text-blue-800 hover:underline flex items-center gap-1 cursor-pointer transition-all focus:outline-none text-left"
                              >
                                {expandedSubjectId === subjectGroup.subjectId ? <ChevronDown className="w-4 h-4 shrink-0" /> : <ChevronRight className="w-4 h-4 shrink-0" />}
                                {subjectGroup.subjectName}
                              </button>
                            ) : (
                              <span className="text-slate-500 flex items-center gap-1">
                                <span className="w-4 h-4 inline-block shrink-0" />
                                {subjectGroup.subjectName} <span className="text-[10px] bg-slate-100 text-slate-400 px-1.5 py-0.5 rounded ml-2 whitespace-nowrap">Chưa mở</span>
                              </span>
                            )}
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className="inline-flex items-center justify-center px-2 py-1 text-xs font-bold rounded-full bg-slate-100 text-slate-700">
                              Kỳ {subjectGroup.semesterIndex}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center">
                            <span className={`text-[11px] px-2 py-1 rounded font-semibold ${typeColor}`}>
                              {typesLabels[subjectGroup.type] || subjectGroup.type}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-center font-bold text-slate-700">
                            {subjectGroup.credits || (subjectGroup.classes[0] as any)?.credits || 3}
                          </td>
                        </tr>
                          
                          {/* Expanded row for classes */}
