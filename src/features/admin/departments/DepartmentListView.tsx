...
export function DepartmentListView({ triggerToast }: DepartmentListViewProps) {
  ...
  const columns = [
    {
      header: 'Mã Khoa',
      accessor: (dept: any) => <span className="font-mono font-bold text-slate-800">{dept.id}</span>
    },
    {
      header: 'Tên Khoa',
      accessor: (dept: any) => <span className="font-medium text-slate-800">{dept.name}</span>
    },
    {
      header: 'Mô Tả',
      accessor: (dept: any) => <p className="text-xs text-slate-500 max-w-sm truncate">{dept.description}</p>
    },
    {
      header: 'Thao Tác',
      accessor: (dept: any) => (
      ...
      <Card className="p-4 flex flex-col sm:flex-row gap-4 items-center">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            placeholder="Tìm theo mã, tên hoặc mô tả khoa..."
